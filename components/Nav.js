...
export default function Nav({ show }) {
    const inactiveLink = 'flex gap-2 items-center p-2 transition-colors duration-150';
    const activeLink = inactiveLink + ' bg-highlight text-black rounded-md shadow';
    const inactiveIcon = 'size-6';
    const activeIcon = 'text-primary size-6';
    ...
    return (
        <div>
            <aside
                className={
                    (show ? 'left-0 debug-active' : '-left-full debug-inactive') +
                    " top-0 text-gray-500 p-4 fixed w-full bg-bgGray h-full md:static md:w-auto transition-all z-40"
                }
            >
                <div className="mb-6 mr-4 flex items-center justify-center">
                    <Logo />
                </div>
                <nav className="flex flex-col gap-2">
                    ...
                    <span onClick={logout} className={inactiveLink + ' cursor-pointer hover:bg-red-100 rounded-md'}>
